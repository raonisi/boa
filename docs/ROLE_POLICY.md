...
- 본인 산하 고객, 계약, 알림, 실적, 후속관리, 상담기록, 우선순위, 태그만 조회하거나 관리할 수 있습니다.
...
- 본인 팀 고객, 계약, 알림, 실적, 후속관리, 상담기록, 우선순위, 태그만 조회하거나 관리할 수 있습니다.
...
- 본인 담당 고객과 관련된 계약, 알림, 실적, 후속관리, 상담기록, 우선순위, 태그만 조회하거나 관리할 수 있습니다.
- 본인 담당 계약에 한해 삭제 요청을 생성할 수 있습니다.
- DB 배정, 사용자 관리, 팀 관리, 업로드 이력 관리, batch 취소, 삭제 데이터 관리, 복구, 완전삭제, 운영 점검, 설정 관리, 데이터 다운로드, 보안 관리 기능은 사용할 수 없습니다.

## 조직 일정 공유 및 변경

- 모든 `active` 역할은 조직 전체 활성 사용자의 일정을 조회할 수 있습니다.
- 일정 조회 권한은 일정 생성, 수정 또는 삭제 권한을 의미하지 않습니다.
- `branch_admin`은 본인과 다른 활성 사용자의 일정을 생성하고 조직 내 모든 일정을 수정하거나 삭제할 수 있습니다.
- `sub_branch_admin`, `team_leader`, `member`는 `schedules.userId`가 본인인 일정만 직접 생성, 수정 또는 삭제할 수 있습니다.
- `createdBy`는 실제 생성자를 기록하는 감사 정보이며 일정 소유권 판단에 사용하지 않습니다.
- `sub_branch_admin`은 직접 산하 팀장·팀원과 산하 팀장의 팀원 일정에 대해 지점장 승인 요청을 만들 수 있습니다.
- `team_leader`는 직접 산하 팀원 일정에 대해서만 지점장 승인 요청을 만들 수 있습니다.
- `member`는 타인 일정 변경 요청을 만들 수 없습니다.
- 일정 변경 요청은 `branch_admin` 승인 전까지 원본 일정에 반영되지 않으며, 승인 시 조직 계층, 활성 상태, 일정 소유자와 원본 변경 여부를 다시 검증합니다.
- 요청 권한은 타인 일정을 직접 생성, 수정 또는 삭제하는 권한이 아닙니다.
- `inactive`, `resigned` 사용자는 일정 조회, 생성, 수정, 삭제 API에 접근할 수 없습니다.
- 현재 하나의 BOA CRM 운영 DB와 Railway 환경은 하나의 독립 조직만 서비스합니다. 멀티테넌시와 별도 조직 식별자는 현재 범위에 포함하지 않습니다.

상세한 요청 상태, 승인 원자성, 충돌 및 안전 로그 정책은 [일정 변경 요청 정책](./SCHEDULE_CHANGE_REQUEST_POLICY.md)을 따릅니다.
