...
| 행동상태  | 기준                                                          |
| --------- | ------------------------------------------------------------- |
| 즉시 확인 | 실패·충돌·인증 이상처럼 관리자가 바로 확인해야 하는 실제 항목 |
| 처리 필요 | 승인 대기·미처리 업무처럼 완료를 위해 조치해야 하는 실제 항목 |
| 참고      | 완료·정상 운영 상태를 확인하는 참고 기록                      |

숫자형 위험점수나 건수 임계값으로 상태를 승격하지 않는다. 각 업무의 실제 상태를 3단계 행동상태로 분류하고, 같은 단계 안에서는 사실 건수와 기한을 정렬 기준으로만 사용한다.
...
3. 종합 행동상태와 운영 작업 요약 카드가 깨지지 않는지 확인한다.
