...
const defaults: Record<string, unknown> = {
  ...
  "customerDataQuality.filterOptions": {
    issueTypes: [{ type: "missing_phone", label: "전화번호 누락", severity: "high" }],
    qualityLevels: [{ value: "good", label: "양호" }],
    assignees: [{ id: 4, name: "[E2E] Member", role: "member", teamId: 10, subBranchAdminId: 2 }],
  ...
  "customerDataQuality.dashboard": {
    scope: { role: "branch_admin", userId: 1, teamId: null, subBranchAdminId: null, canViewAssigneeBreakdown: true },
    summary: {
      customerCount: 1,
  ...
  "managementReports.generate": {
    reportMeta: {
      ...
      scope: { type: "all", label: "전체 조직", targetTeamId: null, targetSubBranchId: null, targetUserId: null },
    },
    summary: {
      activeUserCount: 2,
      customerCount: 1,
    ...
    topIssues: [{ type: "unread_notification", label: "미확인 알림", count: 1, severity: "medium", recommendation: "미확인 알림을 차례로 처리해 주세요." }],
    users: [{
      userId: 4,
      name: "[E2E] Member",
      role: "member",
      ...
      riskLevel: "low",
      coachingPoint: "현재 흐름을 유지하되, 오늘 예정 업무를 먼저 마무리해 주세요.",
    }],
    narrativeSummary: "이번 주 주간 운영 보고서 기준으로 전체 조직 운영 현황을 정리했습니다.",
    copyableSummary: "이번 주 주간 운영 보고서 요약입니다.\n\n• 후속관리 지연: 0건",
    empty: false,
  },
