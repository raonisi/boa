...
- [x] 업로드 batch 이력 조회 및 batch 취소
- [x] 삭제 데이터 복구/완전삭제 관리자 UI
- [x] 테스트로 CSV/XLSX 정책 및 batch 취소/복구 권한 검증
...
### 알려진 제한사항
- 파일 크기 제한 5MB
- 대용량 업로드는 로딩 상태만 제공하며 행/퍼센트 진행률은 아직 미구현
- 월별 활동 로그 아카이빙 자동화는 앱 내부가 아니라 운영 작업으로 별도 설계 필요
- 백업 복구는 soft delete 복구 UI와 별개로 인프라 runbook 정리가 필요
...
3. 고객 일괄 등록 (`/customers/bulk-import`)
   ...
   - 파일 업로드 및 검증
   - 데이터 등록
4. 업로드 이력 확인 (`/customers/import-batches`)
   - batch 상세 조회
   - 취소 가능/불가 조건 확인
5. 삭제 데이터 관리 (`/deleted-data`)
   - soft delete 복구
   - 완전삭제 차단 조건 확인
6. 기존 기능 회귀 테스트

### 3단계: 대용량 업로드 UX 개선
1. `client/src/pages/CustomerBulkImport.tsx`에 진행 상태 UI 추가
2. 필요 시 chunk 단위 서버 처리 설계
3. 대용량 업로드 테스트 케이스 보강
...
### 5단계: 운영 자동화/문서화
1. 활동 로그 월별 아카이빙 정책 정리
2. 백업/복구 runbook 정리
3. 검수 문서와 인수인계 문서의 기능 매트릭스 동기화
