...
const formSchema = z.object({
  ...
  followUpPreset: z
    .enum(["none", "today", "tomorrow", "3days", "1week"])
    .optional(),
...
export function QuickConsultationModal({
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      status: (STATUS_OPTIONS.includes(currentStatus as any)
        ? currentStatus
        : "미상담") as any,
      nextAction: (NEXT_ACTION_OPTIONS.includes(currentNextAction as any)
        ? currentNextAction
        : undefined) as any,
      summary: "",
      followUpPreset: "none",
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    if (values.followUpPreset !== "none") {
      ...
      switch (values.followUpPreset) {
        case "today":
          nextContactAt = format(now, "yyyy-MM-dd");
          break;
        case "tomorrow":
          nextContactAt = format(addDays(now, 1), "yyyy-MM-dd");
          break;
        case "3days":
          nextContactAt = format(addDays(now, 3), "yyyy-MM-dd");
          break;
        case "1week":
          nextContactAt = format(addDays(now, 7), "yyyy-MM-dd");
          break;
      }
    }
