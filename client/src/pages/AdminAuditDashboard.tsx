...
export default function AdminAuditDashboard() {
  ...
  const metric = (key: keyof NonNullable<typeof summary>["cards"]) =>
    Number(summary?.cards?.[key] ?? 0);
  const cautionCount =
  ...
  const health =
    riskCount >= 10
      ? {
          ...
          helper: "위험 작업이 많습니다. 사유 로그 확인이 필요합니다.",
        }
      : cautionCount > 0 || riskCount > 0
        ? {
            label: "주의",
            className: "bg-amber-100 text-amber-800",
            helper: "확인할 운영 항목이 있습니다.",
          }
        : {
            label: "정상",
            className: "bg-emerald-100 text-emerald-700",
            helper: "현재 주요 운영 위험이 낮습니다.",
          };
  const cautionCards = [
    {
    ...
    },
    {
      ...
      icon: Database,
      onClick: () => setLocation("/deleted-data"),
    },
    {
  ...
  const riskCards = [
    {
      key: "recentDownloads",
      label: "최근 다운로드",
      helper: "최근 데이터 다운로드가 많으면 사유 로그를 확인하세요.",
      icon: Download,
    },
    {
    ...
    {
      key: "recentLoginBlocked",
      label: "로그인 차단",
      helper: "계정 상태와 접근 시도를 확인하세요.",
      icon: ShieldCheck,
    },
    {
      key: "recentSecurityActions",
      label: "OAuth/강제 로그아웃",
      helper: "보안 조치 사유를 확인하세요.",
      icon: ShieldCheck,
    },
  ] as const;
  const systemCards = [
    ...
    <DashboardLayout>
      <div className="space-y-5">
        <Card className="border-slate-200/80 bg-white/95 shadow-sm">
          <CardContent className="p-5">
            ...
            <p className="mt-1 text-sm text-slate-500">
              시스템 운영 상태와 위험 작업 이력을 확인합니다. 고객 연락처, 메모,
              토큰, 비밀값은 표시하지 않습니다.
            </p>
          </CardContent>
        </Card>
                ...
                return (
                  ...
                  >
                    ...
                    <Button
                      ...
                      size="sm"
                      variant="outline"
                      className="mt-3 min-h-12 w-full md:h-8 md:min-h-8"
        ...
        <Card className="overflow-hidden border-slate-200/80 bg-white/95 shadow-sm">
          <CardHeader className="pb-2">
            <CardTitle className="flex items-center gap-2 text-sm">
              <AlertTriangle className="h-4 w-4 text-orange-500" /> 최근 위험
              작업
            </CardTitle>
          </CardHeader>
            ...
            <div className="space-y-2 px-4 pb-4 md:hidden">
              {(summary?.recentRiskEvents ?? []).length === 0 ? (
                <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/70 p-4 text-center text-sm text-muted-foreground">
                  최근 위험 작업이 없습니다. 다운로드, 삭제, 복구, 보안 조치가
                  발생하면 이곳에 표시합니다.
                </div>
              ) : (
                summary?.recentRiskEvents.map(entry => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <Badge
                        className={
                          riskClasses[entry.riskLevel] ?? riskClasses.normal
                        }
                      >
                        {riskLabels[entry.riskLevel] ?? riskLabels.normal}
                      </Badge>
                    </div>
        ...
        <Card className="overflow-hidden border-slate-200/80 bg-white/95 shadow-sm">
          ...
          <CardContent className="space-y-3">
            <div className="grid gap-2 md:grid-cols-6">
              <Select
                value={datePreset}
                onValueChange={value => setDatePreset(value as any)}
              >
            ...
            <div className="space-y-2 md:hidden">
              {(logs?.items ?? []).length === 0 ? (
                <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50/70 p-4 text-center text-sm text-muted-foreground">
                  조건에 맞는 활동 로그가 없습니다. 필터를 초기화하거나 기간을
                  넓혀보세요.
                </div>
              ) : (
            ...
            <p className="text-xs text-muted-foreground">
              표시 {logs?.items.length ?? 0}건 / 전체 {logs?.total ?? 0}건
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="border-green-100 bg-green-50/60 shadow-sm">
          <CardContent className="flex items-start gap-3 p-4 text-sm text-muted-foreground">
            ...
            <p>
              다운로드 사유는 데이터 다운로드 화면에서 필수 입력이며,
              DATA_DOWNLOAD 로그 metadata에 저장됩니다. 사유에는 민감정보를
              입력하지 않도록 안내합니다.
            </p>
          </CardContent>
        </Card>
