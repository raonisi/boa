    ...
    <DashboardLayout>
      <div className="space-y-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">조직 구조, 팀 생성, 팀원 배치를 관리합니다.</p>
        </div>
...
function OrgHierarchyView() {
  ...
  const unassignedUsers = activeUsers.filter((u) =>
    u.role !== "branch_admin" && u.role !== "sub_branch_admin" && !(u as any).subBranchAdminId
  );

  const updateSubBranchMutation = trpc.users.updateSubBranchAdmin.useMutation({
    onSuccess: () => { toast.success("부지점장 산하가 변경되었습니다."); utils.users.list.invalidate(); },
  ...
  const updateTeamInfoMutation = trpc.users.updateTeamInfo.useMutation({
    onSuccess: () => { toast.success("팀 정보가 변경되었습니다."); utils.users.teams.invalidate(); utils.users.list.invalidate(); },
    onError: () => toast.error("변경에 실패했습니다."),
  });
      ...
      {subBranchAdmins.length === 0 ? (
        <Card>
          <CardContent className="py-8 text-center text-muted-foreground text-sm">
            ...
            <p>등록된 부지점장이 없습니다.</p>
            <p className="text-xs mt-1">사용자 관리에서 역할을 부지점장으로 변경해주세요.</p>
          </CardContent>
        </Card>
      ) : (
        subBranchAdmins.map((sba) => {
          const sbaTeams = (teams ?? []).filter((t) => (t as any).subBranchAdminId === sba.id);
          const sbaUsers = activeUsers.filter((u) => (u as any).subBranchAdminId === sba.id && u.role !== "sub_branch_admin");
            ...
            <Card key={sba.id} className="border-blue-200">
              <CardHeader className="pb-2 bg-blue-50/50 rounded-t-lg">
                <CardTitle className="text-sm flex items-center gap-2">
                  ...
                  <span className="text-xs font-normal text-muted-foreground">(부지점장) · 팀 {sbaTeams.length}개 · 팀원 {sbaUsers.length}명</span>
                </CardTitle>
              </CardHeader>
              <CardContent className="p-3 space-y-3">
                ...
                {sbaTeams.map((team) => {
                  ...
                  return (
                    <div key={team.id} className="border rounded-lg p-3 bg-white">
                      <div className="flex items-center gap-2 mb-2">
                        <Users className="h-3.5 w-3.5 text-indigo-500" />
                        <span className="text-sm font-semibold text-indigo-700">{team.name}</span>
                        <span className="text-xs text-muted-foreground">팀장: {teamManager?.name ?? "미지정"} · {teamMembers.length}명</span>
                        {/* 팀 부지점장 변경 */}
                        <div className="ml-auto">
                          <Select
                            value={String((team as any).subBranchAdminId ?? "none")}
                            onValueChange={(v) => updateTeamInfoMutation.mutate({ id: team.id, subBranchAdminId: v === "none" ? null : Number(v) })}
                          >
                            <SelectTrigger className="h-6 text-xs w-28"><SelectValue placeholder="부지점장" /></SelectTrigger>
                            <SelectContent>
                      ...
                      {teamMembers.length === 0 ? (
                        <p className="text-xs text-muted-foreground pl-5">팀원이 없습니다.</p>
                      ) : (
                ...
                {unassignedInSba.length > 0 && (
                  <div className="border border-dashed rounded-lg p-3 bg-orange-50/50">
                    <p className="text-xs text-orange-600 font-medium mb-2">팀 미배정 ({unassignedInSba.length}명)</p>
                    <div className="space-y-1 pl-2">
                      {unassignedInSba.map((u) => (
                        <div key={u.id} className="flex items-center gap-2 text-xs">
                          ...
                          >
                            <SelectTrigger className="h-6 text-[10px] w-24 ml-auto"><SelectValue placeholder="팀 배정" /></SelectTrigger>
                            <SelectContent>
                              <SelectItem value="none">팀 없음</SelectItem>
                              {sbaTeams.map((t) => <SelectItem key={t.id} value={String(t.id)}>{t.name}</SelectItem>)}
                            </SelectContent>
                          </Select>
                        </div>
                      ))}
      ...
      {unassignedUsers.length > 0 && (
        <Card className="border-gray-300">
          <CardHeader className="pb-2 bg-gray-50 rounded-t-lg">
            <CardTitle className="text-sm text-gray-600">부지점장 미배정 ({unassignedUsers.length}명)</CardTitle>
          </CardHeader>
                ...
                <div key={u.id} className="flex items-center gap-2 text-sm">
                  ...
                  <div className="ml-auto flex gap-2">
                    ...
                    <Select
                      value="none"
                      onValueChange={(v) => {
                        if (v !== "none") updateSubBranchMutation.mutate({ userId: u.id, subBranchAdminId: Number(v) });
                      }}
                    >
                      <SelectTrigger className="h-7 text-xs w-28"><SelectValue placeholder="부지점장 배치" /></SelectTrigger>
                      <SelectContent>
...
function TeamListView() {
  ...
  return (
    <div className="space-y-4">
      ...
      {(teams ?? []).length === 0 ? (
        ...
        <div className="space-y-4">
          {(teams ?? []).map((team) => {
            const teamMembers = activeUsers.filter((u) => u.teamId === team.id);
            const teamManager = activeUsers.find((u) => u.id === team.managerId);
            const sba = activeUsers.find((u) => u.id === (team as any).subBranchAdminId);
            return (
              <Card key={team.id}>
                <CardHeader className="pb-2">
                  <CardTitle className="text-base flex items-center gap-2">
                    ...
                    <span className="text-xs font-normal text-muted-foreground ml-1">
                      팀장: {teamManager?.name ?? "미지정"} · 부지점장: {sba?.name ?? "미배정"} · 팀원 {teamMembers.length}명
                    </span>
                ...
                <CardContent className="p-0">
                  <table className="w-full text-sm">
                    ...
                    <tbody className="divide-y">
                      {teamMembers.length === 0 ? (
                        <tr><td colSpan={4} className="text-center text-muted-foreground py-4 text-sm">팀원이 없습니다.</td></tr>
                      ) : (
                        teamMembers.map((u) => (
                          <tr key={u.id}>
      ...
      {/* 미배정 팀원 */}
      {(() => {
        const unassigned = activeUsers.filter((u) => !u.teamId && u.role !== "branch_admin" && u.role !== "sub_branch_admin");
        ...
        return (
          <Card>
            ...
            <CardContent className="p-0">
              <table className="w-full text-sm">
                <thead className="border-b"><tr className="text-xs text-muted-foreground"><th className="text-left p-3">이름</th><th className="text-left p-3">역할</th><th className="text-left p-3">팀 배정</th></tr></thead>
                <tbody className="divide-y">
                  {unassigned.map((u) => (
                    <tr key={u.id}>
                      <td className="p-3 font-medium">{u.name ?? "-"}</td>
      ...
      <Dialog open={showCreateTeam} onOpenChange={setShowCreateTeam}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            <div><Label className="text-xs">팀 이름 *</Label><Input value={teamName} onChange={(e) => setTeamName(e.target.value)} className="h-9 mt-1" placeholder="예: 1팀, 영업팀" /></div>
            <div>
...
function EditTeamModal({ team, users, subBranchAdmins, teams, onClose, onSubmit, loading }: {
  team: any; users: any[]; subBranchAdmins: any[]; teams: any[];
  ...
  const managers = users.filter((u) => u.role === "team_leader" || u.role === "branch_admin");
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-3">
          <div><Label className="text-xs">팀 이름</Label><Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="h-9 mt-1" /></div>
          <div><Label className="text-xs">팀 설명</Label><Input value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="h-9 mt-1" /></div>
          <div>
          ...
          <div>
            ...
            <Select value={form.subBranchAdminId} onValueChange={(v) => setForm({ ...form, subBranchAdminId: v })}>
              <SelectTrigger className="h-9 mt-1"><SelectValue /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">미배정</SelectItem>
