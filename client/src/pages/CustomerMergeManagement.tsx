...
export default function CustomerMergeManagement() {
  ...
  const { data: groups, isLoading } = trpc.customerMerge.findDuplicates.useQuery({ search: search || undefined, onlyActive: true });
  const { data: preview } = trpc.customerMerge.preview.useQuery(
    { targetCustomerId: targetCustomerId ?? 0, sourceCustomerId: sourceCustomerId ?? 0 },
    { enabled: !!targetCustomerId && !!sourceCustomerId && targetCustomerId !== sourceCustomerId },
  );
      ...
      <div className="space-y-5 p-4 md:p-6">
        ...
        <Card className="border-slate-200/80 bg-white/95 shadow-sm">
          ...
          <CardContent className="space-y-4">
            ...
            {isLoading ? (
              <div className="py-8 text-center text-sm text-muted-foreground">중복 후보를 확인하는 중입니다.</div>
            ) : (groups ?? []).length === 0 ? (
              <div className="py-8 text-center text-sm text-muted-foreground">동일 연락처 기준 중복 후보가 없습니다.</div>
            ) : (
                      ...
                      <Table>
                        <TableHeader className="bg-white/70">
                          <TableRow>
                            ...
                            <TableHead className="text-right">상담/계약/후속</TableHead>
                          ...
                          {group.candidates.map((customer: any) => (
                            <TableRow key={customer.id}>
                              <TableCell>
                                ...
                                <div className="text-xs text-muted-foreground">{customer.maskedPhone ?? "-"}</div>
                              </TableCell>
                              <TableCell>{customer.consultStatus}</TableCell>
                              <TableCell>{customer.priority}</TableCell>
                              <TableCell className="text-right">
                                {customer.stats?.consultations ?? 0}/{customer.stats?.contracts ?? 0}/{customer.stats?.followUps ?? 0}
                              </TableCell>
                              <TableCell className="text-xs">{customer.createdAt ? new Date(customer.createdAt).toLocaleDateString("ko-KR") : "-"}</TableCell>
                              <TableCell>
                                <div className="flex justify-end gap-2 flex-wrap">
                                  <Button size="sm" variant="outline" onClick={() => setLocation(`/customers/${customer.id}`)}>상세</Button>
                                  {group.candidates.filter((item: any) => item.id !== customer.id).map((source: any) => (
                                    <Button key={source.id} size="sm" variant="outline" onClick={() => selectMerge(customer.id, source.id)}>
                                      <GitMerge className="h-3.5 w-3.5 mr-1" />
                                      #{source.id} 병합
                                    </Button>
                                  ))}
        ...
        <Dialog open={!!targetCustomerId && !!sourceCustomerId} onOpenChange={(open) => { if (!open) resetSelection(); }}>
          <DialogContent className="max-h-[90vh] overflow-y-auto rounded-2xl sm:max-w-3xl">
            ...
            {!preview ? (
              <div className="py-8 text-center text-sm text-muted-foreground">미리보기를 불러오는 중입니다.</div>
            ) : (
              <div className="space-y-4">
                <div className="grid md:grid-cols-2 gap-3">
                  <Card className="border-green-100 bg-green-50/60">
                    ...
                    <CardContent className="text-sm space-y-1">
                      ...
                      <p className="text-muted-foreground">{preview.targetCustomer.maskedPhone ?? "-"}</p>
                  ...
                  <Card className="border-amber-100 bg-amber-50/60">
                    ...
                    <CardContent className="text-sm space-y-1">
                      ...
                      <p className="text-muted-foreground">{preview.sourceCustomer.maskedPhone ?? "-"}</p>
                ...
                <div className="rounded-2xl border border-slate-200 bg-slate-50/70 p-3 text-sm">
                  ...
                  <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 md:grid-cols-4">
                    ...
                    <span>삭제 요청 {preview.transferCounts.deleteRequests}건</span>
                    <span>상태 이력 {preview.transferCounts.statusHistory}건</span>
                    <span>동의 이력 {preview.transferCounts.consentLogs}건</span>
                    <span>배정 이력 {preview.transferCounts.assignmentHistory}건</span>
                  </div>
                </div>
