    ...
    <DashboardLayout>
      <div className="max-w-3xl space-y-6">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="text-sm">
                ...
                <ul className="space-y-1 text-xs text-muted-foreground">
                  <li>
                    다운로드 파일에는 고객명, 연락처, 생년월일, 상품명, 보험료
                    등 민감정보가 포함될 수 있습니다.
                  </li>
                  <li>
                    파일은 업무 목적에 한해 사용하고 안전한 저장소에 보관해
                    주세요.
                  </li>
        ...
        <p className="text-xs text-muted-foreground">
          * 다운로드 사유, 건수, 마스킹 여부는 DATA_DOWNLOAD 로그 metadata에
          기록됩니다.
        </p>
      </div>
        ...
        <DialogContent className="flex max-h-[min(85dvh,42rem)] w-[calc(100vw-1.5rem)] flex-col overflow-hidden p-0 sm:max-w-lg">
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-2 sm:px-6">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {(pendingPreview?.fields ?? []).map(field => (
                  <Badge
                    key={field.key}
                    variant={field.sensitive ? "destructive" : "secondary"}
            ...
            <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs text-amber-900">
              민감정보가 포함될 수 있습니다. 외부 파일 생성 후 보관과 공유에
              주의해 주세요.
            </div>
            ...
            <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 p-3 text-sm">
              ...
              <span>
                <span className="flex items-center gap-1 font-medium">
                  <ShieldCheck className="h-4 w-4" /> 마스킹된 파일로 다운로드
                </span>
                <span className="mt-1 block text-xs text-muted-foreground">
                  이름, 연락처, 생년월일, 상품명, 보험료 등 민감 필드를 서버에서
                  마스킹한 뒤 CSV를 생성합니다.
                </span>
              </span>
            </label>
            ...
            {!maskedDownload && (
              <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-red-200 bg-red-50 p-3 text-sm text-red-950">
                <input
                  ...
                  onChange={event =>
                    setRawExportConfirmed(event.target.checked)
                  }
                  className="mt-1"
                />
                <span>
                  ...
                  <span className="mt-1 block text-xs">
                    마스킹 없이 외부 CSV 파일이 생성됩니다. 업무상 필요한
                    경우에만 사유를 남기고 실행하세요.
                  </span>
                </span>
              </label>
            ...
            <div>
              <Label htmlFor="download-reason" className="text-xs">
                다운로드 사유 *
              </Label>
              <Textarea
                id="download-reason"
