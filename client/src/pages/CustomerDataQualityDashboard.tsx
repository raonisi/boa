...
export default function CustomerDataQualityDashboard() {
  ...
  const [sortBy, setSortBy] = useState<"quality_score_asc" | "last_managed_asc" | "issue_count_desc">("quality_score_asc");
  const [detailsOpen, setDetailsOpen] = useState(true);

  const { data: filterOptions } = trpc.customerDataQuality.filterOptions.useQuery();
  const queryInput = useMemo(() => ({
    issueType: issueType !== "all" ? issueType : undefined,
    qualityLevel: qualityLevel !== "all" ? qualityLevel as "good" | "needs_improvement" | "caution" | "critical" : undefined,
    assignedUserId: assignedUserId !== "all" ? Number(assignedUserId) : undefined,
  ...
  const { data, isLoading, isFetching, isError, refetch } = trpc.customerDataQuality.dashboard.useQuery(queryInput);
    ...
    <DashboardLayout>
      <div className="space-y-5 pb-8">
          ...
          <CardContent className="space-y-4 p-5 md:p-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
              <div className="space-y-2">
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#b99b5f]">Data Quality</p>
                <div className="flex items-center gap-2">
                  ...
                  <h1 className="text-2xl font-bold tracking-tight text-[#1f3b57]">{pageTitle}</h1>
                </div>
            ...
            <Collapsible open={detailsOpen} onOpenChange={setDetailsOpen}>
              <CollapsibleTrigger asChild>
                <Button variant="ghost" className="h-10 w-full justify-between px-0 text-sm text-slate-600 hover:bg-transparent">
                  운영 안내
              ...
              <CollapsibleContent className="rounded-xl border border-teal-200/70 bg-teal-50/70 p-3 text-xs leading-relaxed text-teal-900">
                ...
                <p>보완이 필요한 고객을 확인한 뒤, 기존 고객 상세·후속관리·고객 병합 화면에서 직접 처리해 주세요.</p>
              </CollapsibleContent>
            </Collapsible>
          </CardContent>
        </Card>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <KpiCard title="품질 문제 고객" value={data.summary.issueCustomerCount} />
          <KpiCard title="평균 품질 점수" value={data.summary.averageQualityScore} suffix="점" />
          <KpiCard title="우선 정리 필요" value={data.summary.criticalCustomerCount} />
        </div>

        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          <KpiCard title="전화번호 누락" value={data.summary.missingPhoneCount} />
          <KpiCard title="담당자 없음" value={data.summary.unassignedCustomerCount} />
          <KpiCard title="후속관리 없음" value={data.summary.noFollowUpCount} />
          <KpiCard title="장기 미관리" value={data.summary.longUnmanagedCount} />
        </div>

        <Card className="border-slate-200/80 shadow-sm">
          <CardHeader className="pb-3">
            <CardTitle className="text-base">필터</CardTitle>
            <CardDescription>문제 유형, 품질 등급, 담당자 기준으로 점검 범위를 좁힐 수 있습니다.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            <div className="space-y-2">
              <Label>문제 유형</Label>
              <Select value={issueType} onValueChange={setIssueType}>
                <SelectTrigger className="min-h-10"><SelectValue placeholder="전체" /></SelectTrigger>
                <SelectContent>
            ...
            {filterOptions?.canViewAssigneeBreakdown && (filterOptions.teams.length ?? 0) > 0 ? (
              <div className="space-y-2">
                <Label>팀</Label>
            ...
            <div className="space-y-2">
              <Label>검색</Label>
              <Input className="min-h-10" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="고객명, 담당자, 문제 유형" />
            </div>
            <div className="space-y-2">
              <Label>정렬</Label>
              <Select value={sortBy} onValueChange={(value) => setSortBy(value as typeof sortBy)}>
                ...
                <SelectContent>
                  <SelectItem value="quality_score_asc">품질 점수 낮은 순</SelectItem>
                  <SelectItem value="last_managed_asc">최근 관리 오래된 순</SelectItem>
          ...
          {data.issueTypes.map((issue) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <Badge variant="outline" className={cn("shrink-0", SEVERITY_BADGES[issue.severity])}>
                  {issue.count}
                </Badge>
              </div>
              <p className="mt-3 text-xs text-slate-600">{issue.recommendedAction}</p>
            </button>
          ))}
        </div>
          ...
          <Card className="border-slate-200/80 shadow-sm">
            ...
            <CardContent className="overflow-x-hidden">
              ...
              <div className="space-y-3 md:hidden">
                {data.assignees.map((assignee) => (
                  <div key={assignee.userId} className="rounded-2xl border border-slate-200/80 p-4">
                    <p className="font-semibold text-[#1f3b57]">{assignee.name}</p>
        ...
        <Card className="border-slate-200/80 shadow-sm">
          <CardHeader>
            ...
            <CardDescription>전화번호, 생년월일, 보험료 등 민감 정보는 기본 목록에 표시하지 않습니다.</CardDescription>
          </CardHeader>
          <CardContent>
            {data.customers.length === 0 ? (
              ...
              <>
                    ...
                    <TableBody>
                      {data.customers.map((customer) => (
                        <TableRow key={customer.customerId}>
                          <TableCell className="font-medium">{customer.customerDisplayName}</TableCell>
                          ...
                          <TableCell>
                            <Badge variant="outline" className={QUALITY_LEVEL_BADGES[customer.qualityLevel]}>
                              {customer.qualityScore}
                            </Badge>
                          </TableCell>
                          <TableCell>{formatManagedDate(customer.lastManagedAt)}</TableCell>
                          <TableCell className="max-w-xs text-xs text-slate-600">{customer.recommendedAction}</TableCell>
                          <TableCell>
                ...
                <div className="space-y-3 md:hidden">
                  {data.customers.map((customer) => (
                    <div key={customer.customerId} className="rounded-2xl border border-slate-200/80 p-4">
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          <p className="font-semibold text-[#1f3b57]">{customer.customerDisplayName}</p>
                          <p className="mt-1 text-xs text-slate-500">{customer.assignedUserName} · {customer.status}</p>
                        </div>
                        <Badge variant="outline" className={QUALITY_LEVEL_BADGES[customer.qualityLevel]}>
                      ...
                      <p className="mt-3 text-xs text-slate-600">{customer.recommendedAction}</p>
                      <p className="mt-2 text-xs text-slate-500">마지막 관리: {formatManagedDate(customer.lastManagedAt)}</p>
                      <Link href={customer.links.customerDetail}>
                        <Button className="mt-3 min-h-10 w-full">점검하기</Button>
                      </Link>
                    </div>
                  ))}
                </div>
