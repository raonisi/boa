      ...
      <div className="p-4 md:p-6 space-y-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            고객 DB 일괄 업로드 이력을 확인하고, 운영 이력이 없는 batch를 soft
            delete 방식으로 취소합니다.
          </p>
        </div>
          ...
          <CardContent className="space-y-4">
            <div className="flex flex-col md:flex-row gap-2">
              <div className="relative flex-1">
                ...
                <Input
                  className="pl-8"
                  placeholder="파일명 또는 batch ID 검색"
                  value={search}
                  ...
                  {isLoading ? (
                    ...
                    (batches ?? []).map((batch: any) => (
                      <TableRow key={batch.importBatchId}>
                        ...
                        <TableCell>
                          {batch.uploader?.name ?? batch.uploadedBy}
                        </TableCell>
                        ...
                        <TableCell className="text-right">
                          {batch.totalRows}/{batch.successRows}/
                          {batch.failedRows}
                        </TableCell>
                        <TableCell className="text-right">
                          {batch.duplicateRows}
                        ...
                        <TableCell className="text-right">
                          {batch.activeCustomerCount}
                        </TableCell>
                        <TableCell className="text-right">
                          {batch.cancelledCustomerCount}
                        </TableCell>
                        <TableCell>
                          {batchStatusLabels[batch.status] ?? "기타 상태"}
                        </TableCell>
                        <TableCell>
                          <div className="flex justify-end gap-2">
                            <Button
                              ...
                              variant="outline"
                              onClick={() =>
                                setSelectedBatchId(batch.importBatchId)
                              }
                            >
                            ...
                            <Button
                              ...
                              disabled={
                                ...
                                batch.activeCustomerCount === 0
                              }
                              onClick={() =>
                                setCancelTarget(batch.importBatchId)
                              }
                            >
        ...
        >
          <DialogContent className="max-w-5xl">
            <DialogHeader>
              <DialogTitle>업로드 batch 상세</DialogTitle>
            </DialogHeader>
            {detail && (
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                  <div>
                    ...
                    <p className="font-mono text-xs">
                      {detail.batch.importBatchId}
                    </p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">파일명</p>
                    <p>{detail.batch.fileName ?? "-"}</p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">업로드자</p>
                    <p>
                      {detail.batch.uploader?.name ?? detail.batch.uploadedBy}
                    </p>
                  </div>
                  <div>
                    <p className="text-muted-foreground">상태</p>
                    <p>
                      {batchStatusLabels[detail.batch.status] ?? "기타 상태"}
                    </p>
                  </div>
                </div>
        ...
        >
          <DialogContent>
            <DialogHeader>
              <DialogTitle>업로드 Batch 취소</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground">
                이 batch로 업로드된 고객을 비활성 처리합니다. 완전삭제가 아니라
                soft delete 처리되며, 계약, 일정, 상담기록, 알림 등 운영 이력이
                연결된 고객이 있으면 취소가 차단됩니다. 이 작업은 활동 로그에
                기록됩니다.
              </p>
              <div>
                <Label>취소 사유</Label>
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button
                  variant="destructive"
                  disabled={
                    confirmText !== "BATCH취소" || cancelMutation.isPending
                  }
                  onClick={runCancel}
                >
                  Batch 취소
