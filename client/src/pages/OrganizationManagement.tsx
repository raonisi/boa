...
function descendantIds(
  nodeId: number,
  childrenByParent: Map<number | null, OrgNode[]>
) {
...
export default function OrganizationManagement() {
  ...
  const {
    ...
    const rootItems = (nodes ?? []).filter(
      node => node.role === "branch_admin" || node.parentUserId === null
    );
    const branch =
      (nodes ?? []).find(node => node.role === "branch_admin") ?? null;
    const branchChildren = branch
      ? (byParent.get(branch.id) ?? []).filter(
          node => node.role !== "sub_branch_admin"
        )
      : [];
    ...
    if (isBranchAdmin && branchRoot) {
      return (
        <div className="space-y-3">
          ...
          <OrgSection
            title="지점장 직할"
            description="지점장 바로 아래에 배치된 팀장과 팀원입니다."
            count={branchDirectNodes.length}
          >
          ...
          <OrgSection
            title="부지점장 조직"
            description="부지점장별 산하 팀장과 직할 팀원을 확인합니다."
            count={subBranchNodes.length}
          >
          ...
          <OrgSection
            title="미배정 사용자"
            description="상위자가 없고 fallback 소속도 없는 활성 사용자입니다."
            count={unassignedNodes.length}
            defaultOpen={unassignedNodes.length > 0}
          >
    ...
    <DashboardLayout>
      <div className="space-y-6 pb-24 md:pb-8">
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6">
          <SummaryCard
            label="부지점장"
            value={summary.subBranchAdminCount ?? 0}
          />
          <SummaryCard
            label="직할 팀장"
            value={summary.directTeamLeaderCount ?? 0}
          />
          <SummaryCard
            label="전체 팀장"
            value={summary.totalTeamLeaderCount ?? 0}
          />
          <SummaryCard
            label="전체 팀원"
            value={summary.totalMemberCount ?? 0}
          />
          <SummaryCard
            label="직할 팀원"
            value={summary.directMemberCount ?? 0}
          />
        ...
        <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
          ...
          <Card className={`rounded-3xl shadow-sm ${adminPanel.warningSoft}`}>
            ...
            <CardContent className="space-y-3 text-sm text-foreground/90">
              ...
              <p>
                순환 구조와 자기 자신을 상위자로 지정하는 변경은 서버에서
                차단합니다.
              </p>
            </CardContent>
          </Card>
          ...
          {selectedUser ? (
            <div className="space-y-4">
              ...
              <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
                이 변경은 고객 조회 범위와 DB 배분 가능 범위에 영향을 줄 수
                있습니다. 기존 고객 담당자와 상담/계약 이력은 변경되지 않습니다.
              </div>
