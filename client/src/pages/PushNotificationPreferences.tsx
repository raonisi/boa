...
export default function PushNotificationPreferences() {
  ...
  const { data: preferences, isLoading } = trpc.pushNotifications.getPreferences.useQuery();
      ...
      <div className="space-y-5">
        <Card className="border-slate-200/80 bg-white/95 shadow-sm">
          <CardContent className="p-5">
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#b99b5f]">Push Preferences</p>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  알림을 꺼도 BOA CRM 앱과 웹 사용에는 영향이 없습니다. 잠금화면 알림에는 고객명, 전화번호, 보험정보가 표시되지 않습니다.
                </p>
              </div>
              <Badge className="w-fit bg-slate-100 text-slate-700">Asia/Seoul</Badge>
            </div>
          </CardContent>
        </Card>

        {isLoading || !preferences ? (
          <Card><CardContent className="p-8 text-center text-sm text-slate-500">알림 설정을 불러오는 중입니다.</CardContent></Card>
        ) : (
          <>
            <Card className="border-slate-200/80 bg-white/95 shadow-sm">
              <CardHeader>
            ...
            <Card className="border-slate-200/80 bg-white/95 shadow-sm">
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between gap-4 rounded-xl bg-slate-50 p-4">
                  <div>
                    <p className="font-medium text-slate-900">조용한 시간대 사용</p>
                    <p className="mt-1 text-sm text-slate-500">기본값은 21:00부터 08:00까지입니다.</p>
                  </div>
                  <Switch
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(event) => update({ timezone: event.target.value || "Asia/Seoul" })}
                      className="mt-1"
                    />
                  </div>
                </div>
        ...
        <Card className="border-blue-100 bg-blue-50/80">
          <CardContent className="p-4 text-sm text-blue-800">
            업무 알림은 고객정보를 잠금화면에 표시하지 않는 고정 문구로만 발송됩니다. 알림 설정은 본인 계정에만 적용됩니다.
          </CardContent>
        </Card>
      </div>
    </DashboardLayout>
