...
beforeEach(() => {
  ...
  vi.spyOn(db, "getUserById").mockImplementation(async (id: number) => {
    ...
    if (id === 99) return { id: 99, name: "[TEST] Other Agent", teamId: 10 } as any;
    return undefined;
  });
});
...
describe("customerRelationships RBAC", () => {
  it("allows branch_admin to create relationships across scope", async () => {
    ...
    vi.spyOn(relationshipsDb, "findActiveRelationshipDuplicate").mockResolvedValue(
      null
    );
    const createSpy = vi
  ...
  it("allows member to create only for own assigned customer anchor", async () => {
    ...
    await appRouter.createCaller(createCtx("member", { id: 4 })).customerRelationships.create({
      customerId: 100,
      relatedCustomerId: 101,
  ...
  it("blocks member when anchor customer is not theirs", async () => {
    ...
    await expect(
      appRouter.createCaller(createCtx("member", { id: 4 })).customerRelationships.create({
        customerId: 100,
        relatedCustomerId: 101,
        relationshipType: "referral",
      })
...
describe("customerRelationships validation", () => {
  ...
  it("blocks self relationship", async () => {
    await expect(
      appRouter.createCaller(createCtx("branch_admin")).customerRelationships.create({
        customerId: 100,
  ...
  it("does not store note or customer names in activity logs", async () => {
    ...
    await appRouter.createCaller(createCtx("branch_admin")).customerRelationships.create({
      customerId: 100,
      relatedCustomerId: 101,
      relationshipType: "family_spouse",
    });
