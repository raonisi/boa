...
function sanitizeLogText(value: string, maxLength = 160) {
  const sanitized = value
    ...
    .replace(/\b01[016789][-\s.]?\d{3,4}[-\s.]?\d{4}\b/g, (match) => maskPhone(match))
    .replace(/\b02[-\s.]?\d{3,4}[-\s.]?\d{4}\b/g, (match) => {
      const digits = match.replace(/\D/g, "");
      return `${digits.slice(0, 2)}-***-${digits.slice(-4)}`;
    })
    .replace(/\b([A-Z0-9._%+-])([A-Z0-9._%+-]*)(@[A-Z0-9.-]+\.[A-Z]{2,})\b/gi, (_match, first, rest, domain) => `${first}${"*".repeat(Math.max(3, String(rest).length))}${domain}`)
    .replace(/\b(?:token|accessToken|refreshToken|idToken|firebaseToken|deviceToken|fcmToken|secret|clientSecret|password|api[_-]?key|privateKey|DATABASE_URL|JWT_SECRET|authorization|cookie|session|credential|keyFile|googleClientSecret|firebaseAdmin)\s*[:=]\s*[^,\s"}]+/gi, "[REDACTED]");
...
function sanitizeLogValue(value: unknown): unknown {
  ...
  for (const [key, raw] of Object.entries(value as Record<string, unknown>)) {
    ...
    if (/(password|pass|token|accesstoken|refreshtoken|idtoken|firebasetoken|devicetoken|fcmtoken|secret|clientsecret|apikey|privatekey|serviceaccount|database_url|jwt_secret|authorization|cookie|session|credential|keyfile|googleclientsecret|firebaseadmin|openid)/i.test(normalizedKey)) {
      result[key] = "[REDACTED]";
    } else if (/(ssn|residentnumber|rrn|policy_number|policynumber)/i.test(normalizedKey)) {
      result[key] = "[REDACTED]";
    } else if (/birth(date|day)?/i.test(normalizedKey) && typeof raw === "string") {
      result[key] = maskBirthDateForLogs(raw);
    } else if (/phone|contact|mobile|tel/i.test(normalizedKey) && typeof raw === "string") {
    ...
    } else if (/(content|body|scriptbody|templatebody|description|memo|message|note|productname|diseasename|illness|medical)/i.test(normalizedKey)) {
...
function sanitizeActivityLogRow<T extends { details?: string | null }>(entry: T): T {
  return {
    ...entry,
    details: sanitizeActivityLogDetails(entry.details),
    ...("ipAddress" in entry ? { ipAddress: entry.ipAddress ? "[REDACTED]" : entry.ipAddress } : {}),
    ...("userAgent" in entry ? { userAgent: entry.userAgent ? sanitizeLogText(String(entry.userAgent), 80) : entry.userAgent } : {}),
  };
}
...
export const appRouter = router({
  ...
  // ── Auth ──────────────────────────────────────────────────────────────────
  auth: router({
    me: publicProcedure.query((opts) => opts.ctx.user?.accountStatus === "active" ? opts.ctx.user : null),
