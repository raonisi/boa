...
async function verifyManagerAccess(
  ...
  if (currentUser.role === "member") {
    throw new TRPCError({
      code: "FORBIDDEN",
      message: "Members cannot create coaching notes.",
    });
  }
  ...
  if (visibleUserIds && !visibleUserIds.includes(targetUserId)) {
    throw new TRPCError({
      code: "FORBIDDEN",
      message: "Target user is outside of your management scope.",
    });
  }
}
...
export const teamCoachingRouter = router({
    ...
    .mutation(async ({ ctx, input }: { ctx: any; input: any }) => {
      ...
      if (!targetUser || targetUser.accountStatus !== "active") {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message:
            "Cannot create coaching notes for inactive or resigned users.",
        });
      }
      ...
      if (sensitivePattern.test(input.note)) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message:
            "코칭 메모 본문에 고객의 전화번호, 주민등록번호 또는 질병명 등을 직접 입력할 수 없습니다. 안전한 코칭 기록을 위해 관련 정보를 지표나 고객 번호로 대체해 주세요.",
        });
      }
      ...
      const [result] = await db
        .insert(teamMemberCoachingNotes)
        .values(noteData);
  ...
  update: protectedProcedure
    .input(
      z.object({
        ...
        visibility: z
          .enum(["private_admin", "manager_visible", "member_visible"])
          .optional(),
        nextReviewAt: z.string().optional(),
      })
    )
    .mutation(async ({ ctx, input }: { ctx: any; input: any }) => {
      ...
      // Check permissions
      if (
        ctx.user.id !== existingNote.authorUserId &&
        ...
        throw new TRPCError({
          code: "FORBIDDEN",
          message:
            "Only the author or higher-level administrators can update coaching notes.",
        });
      }
      ...
      if (input.actionItems !== undefined)
        updateData.actionItems = input.actionItems;
      if (input.priority) updateData.priority = input.priority;
      if (input.visibility) updateData.visibility = input.visibility;
      if (input.nextReviewAt !== undefined)
        updateData.nextReviewAt = input.nextReviewAt
          ? new Date(input.nextReviewAt)
          : null;

      await db
        .update(teamMemberCoachingNotes)
        .set(updateData)
        .where(eq(teamMemberCoachingNotes.id, input.id));
      ...
      await logCoachingActivity(
        ...
        ctx.user,
        input.id,
        existingNote.targetUserId,
        {
          updatedFields: Object.keys(updateData).filter(
            k => k !== "note" && k !== "actionItems"
          ),
        }
      );
  ...
  list: protectedProcedure
    ...
    .query(async ({ ctx, input }: { ctx: any; input: any }) => {
      ...
      if (ctx.user.role === "member") {
        ...
        conditions.push(
          eq(teamMemberCoachingNotes.visibility, "member_visible")
        );
      } else {
        ...
        if (input.targetUserId) {
          ...
          conditions.push(
            eq(teamMemberCoachingNotes.targetUserId, input.targetUserId)
          );
        } else if (visibleUserIds) {
          conditions.push(
            inArray(teamMemberCoachingNotes.targetUserId, visibleUserIds)
          );
        }
      ...
      const results = await db
        .select({
          ...
          linkedMetricSnapshotJson:
            teamMemberCoachingNotes.linkedMetricSnapshotJson,
  ...
  summary: protectedProcedure.query(async ({ ctx }: { ctx: any }) => {
    const db = await getDb();
    if (!db) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
    ...
    const summary = {
      ...
      thisWeekNotes: results.filter(r => new Date(r.createdAt) >= oneWeekAgo)
        .length,
      dueForReview: results.filter(
        r =>
          r.status === "open" &&
          r.nextReviewAt &&
          new Date(r.nextReviewAt) <= now
      ).length,
      highPriority: results.filter(
