...
describe("notification priority UI utils", () => {
  it("classifies urgent types first", () => {
    expect(classifyNotificationPriority({ type: "schedule_incomplete" })).toBe(
      "urgent"
    );
  ...
  it("classifies today types and overdue dueAt as today", () => {
    expect(classifyNotificationPriority({ type: "schedule_today" })).toBe(
      "today"
    );
  ...
  it("falls back to general when no urgent/today conditions", () => {
    ...
    expect(classifyNotificationPriority({ type: "customer_assigned" })).toBe(
      "general"
    );
  });
