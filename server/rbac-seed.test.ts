...
const users = [
  ...
  {
    id: 41,
    openId: "seed-member-a1",
    name: "[TEST] Member A1",
    email: "member-a1@test.local",
    role: "member",
    accountStatus: "active",
    teamId: 101,
    subBranchAdminId: 21,
  ...
  {
    id: 42,
    openId: "seed-member-a2",
    name: "[TEST] Member A2",
    email: "member-a2@test.local",
    role: "member",
    accountStatus: "active",
    teamId: 101,
    subBranchAdminId: 21,
  ...
  {
    id: 43,
    openId: "seed-member-b1",
    name: "[TEST] Member B1",
    email: "member-b1@test.local",
    role: "member",
    accountStatus: "active",
    teamId: 102,
    subBranchAdminId: 22,
  ...
  {
    id: 98,
    openId: "seed-resigned",
    name: "[TEST] Resigned",
    email: "resigned@test.local",
    role: "member",
...
function setupSeedDb() {
  ...
  vi.spyOn(db, "getUsersBySubBranchAdminId").mockImplementation(
    async (id: number) => users.filter(u => u.subBranchAdminId === id)
  );
  vi.spyOn(db, "getUsersByTeamId").mockImplementation(async (id: number) =>
    users.filter(u => u.teamId === id)
  );
  vi.spyOn(db, "getAllTeams").mockResolvedValue(teams);
  vi.spyOn(db, "getTeamById").mockImplementation(
    async (id: number) => teams.find(t => t.id === id) ?? null
  );
  vi.spyOn(db, "getCustomers").mockImplementation(async (filter: any = {}) =>
    customers.filter(
      c =>
        c.isActive &&
        (filter.subBranchAdminId === undefined ||
          c.subBranchAdminId === filter.subBranchAdminId) &&
        (filter.teamId === undefined || c.assignedTeamId === filter.teamId) &&
        (filter.agentIds === undefined ||
          filter.agentIds.includes(c.agentId)) &&
        (filter.agentId === undefined || c.agentId === filter.agentId)
    )
  );
  vi.spyOn(db, "getCustomerById").mockImplementation(
    async (id: number) => customers.find(c => c.id === id) ?? null
  );
  vi.spyOn(db, "getAllContracts").mockImplementation(async (filter: any = {}) =>
    contracts.filter(contract => {
      ...
      return (
        contract.isActive &&
        (filter.subBranchAdminId === undefined ||
          customer?.subBranchAdminId === filter.subBranchAdminId) &&
        (filter.teamId === undefined ||
          customer?.assignedTeamId === filter.teamId) &&
        (filter.agentIds === undefined ||
          filter.agentIds.includes(contract.agentId)) &&
        (filter.agentId === undefined || contract.agentId === filter.agentId)
      );
    })
  );
  vi.spyOn(db, "getContractById").mockImplementation(
    async (id: number) => contracts.find(c => c.id === id) ?? null
  );
  ...
  vi.spyOn(db, "getSchedules").mockImplementation(async (filter: any = {}) =>
    schedules.filter(
      s =>
        s.isActive &&
        (filter.subBranchAdminId === undefined ||
          users.find(u => u.id === s.userId)?.subBranchAdminId ===
  ...
  vi.spyOn(db, "getNotificationById").mockImplementation(
    async (id: number) => notifications.find(n => n.id === id) ?? null
  );
...
describe("seed-backed RBAC integration", () => {
  it("scopes list queries by seeded organization hierarchy", async () => {
    setupSeedDb();
    await expect(
      appRouter.createCaller(ctx(1)).customers.list({})
    ).resolves.toHaveLength(4);
    await expect(
      appRouter.createCaller(ctx(21)).customers.list({})
    ).resolves.toEqual(
    ...
    await expect(
      appRouter.createCaller(ctx(31)).customers.list({})
    ).resolves.toHaveLength(2);
    await expect(
      appRouter.createCaller(ctx(41)).customers.list({})
    ).resolves.toEqual([expect.objectContaining({ id: 1001 })]);
    await expect(
      appRouter.createCaller(ctx(99)).customers.list({})
    ).rejects.toThrow();
  });
  ...
  it("blocks direct customer and contract access outside the actor scope", async () => {
    setupSeedDb();
    await expect(
      appRouter.createCaller(ctx(41)).customers.get({ id: 1002 })
    ).rejects.toThrow();
    await expect(
      appRouter.createCaller(ctx(21)).customers.get({ id: 2001 })
    ).rejects.toThrow();
    await expect(
      appRouter
        .createCaller(ctx(21))
        .contracts.contractHistory({ contractId: 502 })
    ).rejects.toThrow();
  });
  ...
  it("blocks cross-scope performance, notification, schedule, and assignment mutations", async () => {
    setupSeedDb();
    await expect(
      appRouter.createCaller(ctx(31)).performance.stats({ teamIdFilter: 102 })
    ).rejects.toThrow();
    await expect(
      appRouter.createCaller(ctx(41)).performance.agentStats({ agentId: 42 })
    ).rejects.toThrow();
    await expect(
      appRouter.createCaller(ctx(31)).schedules.create({
    ...
    await expect(
      appRouter
        .createCaller(ctx(31))
        .customers.assign({ customerId: 1001, agentId: 43 })
    ).rejects.toThrow();
  });

  it("returns role-scoped organization trees and blocks member organization access", async () => {
    setupSeedDb();
    const branchTree = await appRouter
      .createCaller(ctx(1))
      .users.organizationTree();
    expect(branchTree.nodes.map(node => node.id)).toEqual(
      expect.arrayContaining([21, 22, 31, 41, 43])
    );

    const subTree = await appRouter
      .createCaller(ctx(21))
      .users.organizationTree();
    expect(subTree.nodes.map(node => node.id)).toEqual(
      expect.arrayContaining([21, 31, 41, 42])
    );
    expect(subTree.nodes.map(node => node.id)).not.toContain(43);

    const leaderTree = await appRouter
      .createCaller(ctx(31))
      .users.organizationTree();
    expect(leaderTree.nodes.map(node => node.id)).toEqual(
      expect.arrayContaining([31, 41, 42])
    );
    expect(leaderTree.nodes.map(node => node.id)).not.toContain(43);

    await expect(
      appRouter.createCaller(ctx(41)).users.organizationTree()
    ).rejects.toThrow();
  });
  ...
  it("allows branch_admin to update supported hierarchy parent shapes", async () => {
    setupSeedDb();
    const updateSpy = vi
      .spyOn(db, "updateUserOrganization")
      .mockResolvedValue(undefined);
    const logSpy = vi.mocked(db.createActivityLog);

    await appRouter
      .createCaller(ctx(1))
      .users.updateParent({ userId: 31, parentUserId: 1 });
    expect(updateSpy).toHaveBeenCalledWith(
      31,
      expect.objectContaining({ parentUserId: 1 })
    );

    await appRouter
      .createCaller(ctx(1))
      .users.updateParent({ userId: 41, parentUserId: 1 });
    expect(updateSpy).toHaveBeenCalledWith(
      41,
      expect.objectContaining({ parentUserId: 1 })
    );
    ...
    await appRouter
      .createCaller(ctx(1))
      .users.updateParent({ userId: 31, parentUserId: 21 });
    expect(updateSpy).toHaveBeenCalledWith(
      31,
      expect.objectContaining({ parentUserId: 21 })
    );

    await appRouter
      .createCaller(ctx(1))
      .users.updateParent({ userId: 42, parentUserId: 21 });
    expect(updateSpy).toHaveBeenCalledWith(
      42,
      expect.objectContaining({ parentUserId: 21 })
    );
    ...
    await appRouter
      .createCaller(ctx(1))
      .users.updateParent({ userId: 42, parentUserId: 31 });
    expect(updateSpy).toHaveBeenCalledWith(
      42,
      expect.objectContaining({ parentUserId: 31 })
    );

    expect(logSpy.mock.calls.at(-1)?.[0]).toMatchObject({
      action: "USER_ORG_PARENT_CHANGED",
    });
  });
  ...
  it("blocks invalid, inactive, resigned, and circular hierarchy parent assignments", async () => {
    ...
    await expect(
      appRouter
        .createCaller(ctx(1))
        .users.updateParent({ userId: 31, parentUserId: 31 })
    ).rejects.toThrow();
    await expect(
      appRouter
        .createCaller(ctx(41))
    ...
    await expect(
      appRouter
        .createCaller(ctx(1))
        .users.updateParent({ userId: 31, parentUserId: 41 })
    ).rejects.toThrow();
    await expect(
      appRouter
        .createCaller(ctx(1))
        .users.updateParent({ userId: 31, parentUserId: 98 })
    ).rejects.toThrow();
    await expect(
      appRouter
        .createCaller(ctx(1))
        .users.updateParent({ userId: 31, parentUserId: 99 })
    ).rejects.toThrow();
    ...
    vi.spyOn(db, "getUserById").mockImplementation(
      async (id: number) => cyclicUsers.find(u => u.id === id) ?? null
    );
    await expect(
      appRouter
        .createCaller(ctx(1))
  ...
  it("allows sub_branch_admin to assign only to descendant users", async () => {
    ...
    await appRouter
      .createCaller(ctx(21))
      .customers.assign({ customerId: 1002, agentId: 31 });
    expect(assignTeamSpy).toHaveBeenCalledWith(1002, 101, 21, {});

    await appRouter
      .createCaller(ctx(21))
      .customers.assign({ customerId: 1002, agentId: 41 });
    expect(assignSpy).toHaveBeenCalledWith(1002, 41, 101, 21, {});

    await expect(
      appRouter
        .createCaller(ctx(21))
        .customers.assign({ customerId: 1002, agentId: 43 })
    ).rejects.toThrow();
  });
  ...
  it("allows team_leader to assign customers only to descendant members", async () => {
    ...
    await expect(
      appRouter
        .createCaller(ctx(41))
        .customers.assign({ customerId: 1001, agentId: 42 })
    ).rejects.toThrow();
  });
  ...
  it("blocks empty, unsupported, oversized, or mismatched bulk import payloads", async () => {
    ...
    await expect(
      appRouter
        .createCaller(ctx(41))
        .customers.bulkImport({ rows: [], fileName: "customers.csv" })
    ).rejects.toThrow();
    await expect(
      appRouter
        .createCaller(ctx(1))
...
describe("OAuth pre-registration guard", () => {
  ...
  it("links an invited pre-registered user to a Google sub on first login", async () => {
    ...
    vi.spyOn(sdk, "getGoogleUserInfo").mockResolvedValue({
      sub: "google-member-a1",
      email: "member-a1@test.local",
      email_verified: true,
      name: "[TEST] Member A1",
    } as any);
    vi.spyOn(db, "getAllUsersByEmail").mockResolvedValue([
      { ...users[5], openId: "invited_member_a1", loginStatus: "invited" },
    ] as any);
    vi.spyOn(db, "getUserByOpenId")
      .mockResolvedValueOnce(undefined)
      .mockResolvedValueOnce({
        ...users[5],
        openId: "google-member-a1",
        loginStatus: "linked",
      } as any);
  ...
  it("blocks overwriting an already linked openId", async () => {
    ...
    vi.spyOn(db, "getAllUsersByEmail").mockResolvedValue([
      { ...users[5], openId: "existing-google-sub", loginStatus: "linked" },
    ] as any);
    const upsertSpy = vi.spyOn(db, "upsertUser").mockResolvedValue(undefined);
    vi.spyOn(db, "createActivityLog").mockResolvedValue(undefined);
