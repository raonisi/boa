...
async function canAccessCustomerSilent(
  ...
  if (user.role === "sub_branch_admin") return customer.subBranchAdminId === user.id;
  if (user.role === "team_leader") {
    if (customer.assignedTeamId && customer.assignedTeamId === user.teamId) return true;
...
function parseOptionalScheduleDate(value: string | undefined, fieldName: string) {
  if (!value) return undefined;
  const parsed = parseKstLocalDateTime(value.includes("T") ? value : `${value}T00:00:00`);
...
function parseOptionalScheduleEndDate(value: string | undefined) {
  if (!value) return undefined;
  const parsed = parseKstLocalDateTime(value.includes("T") ? value : `${value}T23:59:59`);
  if (Number.isNaN(parsed.getTime())) {
    throw new TRPCError({ code: "BAD_REQUEST", message: "종료일이 올바르지 않습니다." });
  }
  return parsed;
}
  ...
  switch (viewMode) {
    ...
    case "user": {
      if (!input.ownerUserId) {
        throw new TRPCError({ code: "BAD_REQUEST", message: "조회할 직원을 선택해주세요." });
      }
      const target = activeUsers.find((user) => user.id === input.ownerUserId);
      if (!target) {
        throw new TRPCError({ code: "NOT_FOUND", message: "선택한 직원을 찾을 수 없습니다." });
      }
      return [target.id];
    }
    case "team": {
      if (!input.teamId) {
        throw new TRPCError({ code: "BAD_REQUEST", message: "조회할 팀을 선택해주세요." });
      }
      return activeUsers.filter((user) => user.teamId === input.teamId).map((user) => user.id);
    }
    case "organization":
...
export async function listCalendarSchedules(
  ...
  const teamNameById = new Map(allTeams.map((team) => [team.id, team.name ?? `팀 #${team.id}`]));
  const userNameById = new Map(activeUsers.map((user) => [user.id, user.name ?? `사용자 #${user.id}`]));

  const ownerUserIds = await resolveScheduleOwnerUserIds(viewer, input, activeUsers);
  const dateFrom = parseOptionalScheduleDate(input.dateFrom, "시작일");
  const dateTo = parseOptionalScheduleEndDate(input.dateTo);

  const scheduleFilter = {
    ...(ownerUserIds.length === 1 ? { userId: ownerUserIds[0] } : { userIds: ownerUserIds }),
    dateFrom,
    dateTo,
  };

  const rawSchedules = ownerUserIds.length === 0
    ? []
    : await getSchedules(scheduleFilter);

  const filteredSchedules = rawSchedules.filter((schedule) => activeUserIds.has(schedule.userId));
  const mutableScheduleIds = new Set((await getAccessibleSchedules(viewer)).map((schedule) => schedule.id));

  const schedules: CalendarScheduleItem[] = await Promise.all(filteredSchedules.map(async (schedule) => {
    ...
    if (schedule.customerId) {
      canViewCustomerDetail = await canAccessCustomerSilent(viewer, schedule.customerId);
    ...
    const item: CalendarScheduleItem = {
      ...
      ownerName: userNameById.get(schedule.userId) ?? `사용자 #${schedule.userId}`,
  ...
  if (viewMode === "organization" && schedules.length >= ORGANIZATION_VIEW_WARNING_THRESHOLD) {
    organizationViewWarning = "전체 일정이 많습니다. 기간을 좁히면 더 빠르게 확인할 수 있습니다.";
  }
