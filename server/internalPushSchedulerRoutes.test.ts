...
async function withInternalSchedulerServer<T>(
...
describe("internal push scheduler HTTP route", () => {
  it("returns a safe summary for an authorized scheduler trigger", async () => {
    ...
    const engineSpy = vi
      .spyOn(pushNotifications, "runPushReminderEngines")
      .mockResolvedValue({
        success: true,
        targetCount: 0,
        sentCount: 0,
    ...
    await withInternalSchedulerServer(async baseUrl => {
      const response = await fetch(
        `${baseUrl}/api/internal/push-reminders/run`,
        {
          method: "POST",
          headers: {
      ...
      expect(JSON.stringify(body)).not.toMatch(
        /results|010|birthDate|productName|monthlyPremium|raw-token/i
      );
    });
