function maskLogPhone(value: string) {
  ...
  if (digits.startsWith("02")) return `${digits.slice(0, 2)}-***-${digits.slice(-4)}`;
...
export function sanitizeActivityLogText(value: string, maxLength = 160) {
  const sanitized = value
    ...
    .replace(/\b01[016789][-\s.]?\d{3,4}[-\s.]?\d{4}\b/g, (match) => maskLogPhone(match))
...
export function sanitizeActivityLogValue(value: unknown): unknown {
  ...
  for (const [key, item] of Object.entries(value as Record<string, unknown>)) {
    ...
    if (/(password|pass|token|accesstoken|refreshtoken|idtoken|firebasetoken|devicetoken|fcmtoken|secret|clientsecret|apikey|privatekey|serviceaccount|database_url|jwt_secret|authorization|cookie|session|credential|keyfile|googleclientsecret|firebaseadmin|openid|ssn|residentnumber|rrn|policy_number|policynumber)/i.test(normalizedKey)) {
      ...
      result[key] = digits.length === 6 ? `${digits.slice(0, 2)}****` : digits.length >= 8 ? `${digits.slice(0, 4)}****` : "[masked-birth-date]";
    } else if (/phone|contact|mobile|tel/i.test(normalizedKey) && typeof item === "string") {
...
export function sanitizeActivityLogRow<T extends { details?: string | null }>(entry: T): T {
  return {
    ...entry,
    details: sanitizeActivityLogDetailsForStorage(entry.details),
    ...("ipAddress" in entry ? { ipAddress: (entry as any).ipAddress ? "[REDACTED]" : (entry as any).ipAddress } : {}),
    ...("userAgent" in entry ? { userAgent: (entry as any).userAgent ? sanitizeActivityLogText(String((entry as any).userAgent), 80) : (entry as any).userAgent } : {}),
  };
}
