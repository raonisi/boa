...
export const branchAdminProcedure = protectedProcedure.use(({ ctx, next }) => {
  ...
  if (u.role !== "branch_admin")
    throw new TRPCError({
      code: "FORBIDDEN",
      message: "지점장만 접근 가능합니다.",
    });
  return next({ ctx });
});

/** 부지점장 이상 (sub_branch_admin, branch_admin + active) */
export const subBranchAdminOrAboveProcedure = protectedProcedure.use(
  ({ ctx, next }) => {
    const u = ctx.user;
    if (u.accountStatus !== "active")
      throw new TRPCError({
...
export const teamLeaderOrAboveProcedure = protectedProcedure.use(
  ({ ctx, next }) => {
    const u = ctx.user;
    if (u.accountStatus !== "active")
      throw new TRPCError({
...
export const customerBulkImportProcedure = protectedProcedure.use(
  ({ ctx, next }) => {
    const u = ctx.user;
    if (u.accountStatus !== "active")
      throw new TRPCError({
...
export const managerAnalyticsProcedure = protectedProcedure.use(
  ({ ctx, next }) => {
    const u = ctx.user;
    if (u.accountStatus !== "active")
      throw new TRPCError({
    ...
    if (
      ...
      throw new TRPCError({
        code: "UNAUTHORIZED",
        message: "관리자(지점장·부지점장·팀장)만 접근할 수 있습니다.",
      });
    }
    return next({ ctx });
  }
