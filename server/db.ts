...
export async function createActivityLog(data: InsertActivityLog, client?: DbExecutor) {
  ...
  await db.insert(activityLogs).values({
    ...
    userAgent: data.userAgent ? sanitizeActivityLogText(data.userAgent, 80) : data.userAgent,
  });
}
...
function truncateTimelineText(value: unknown, maxLength = 120) {
  ...
  const normalized = sanitizeActivityLogText(value.replace(/\s+/g, " ").trim(), maxLength);
  if (!normalized) return null;
