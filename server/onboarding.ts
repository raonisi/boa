...
async function assertCanManageTarget(actor: AppUser, targetUserId: number) {
  ...
  if (target.accountStatus !== "active")
    throw new TRPCError({
      code: "BAD_REQUEST",
      message: "inactive/resigned 사용자는 신규 배정할 수 없습니다.",
    });

  if (actor.role === "branch_admin") return target;
  if (actor.role === "sub_branch_admin") {
    const ids = await getHierarchyScopeUserIds(actor);
    if (!ids || !ids.includes(targetUserId))
      throw new TRPCError({
        code: "FORBIDDEN",
        message: "산하 직원에게만 배정할 수 있습니다.",
      });
    return target;
  }
  if (actor.role === "team_leader") {
    const ids = await getHierarchyScopeUserIds(actor);
    if (!ids || !ids.includes(targetUserId) || target.role !== "member")
      throw new TRPCError({
        code: "FORBIDDEN",
        message: "산하 팀원에게만 배정할 수 있습니다.",
      });
    return target;
  }
...
async function assertCanReadAssignment(
  actor: AppUser,
  assignment: { targetUserId: number }
) {
  if (actor.role === "branch_admin") return;
  if (actor.role === "member") {
    if (assignment.targetUserId !== actor.id)
      throw new TRPCError({ code: "FORBIDDEN" });
    return;
  }
  const ids = await getHierarchyScopeUserIds(actor);
  if (ids && !ids.includes(assignment.targetUserId))
    throw new TRPCError({ code: "FORBIDDEN" });
}
...
async function recalcAssignmentProgress(assignmentId: number) {
  ...
  const now = new Date();
  const isOverdue =
    (assignment.dueAt?.getTime?.() ?? new Date(assignment.dueAt).getTime()) <
      now.getTime() && requiredPending > 0;
  const status =
    requiredPending === 0
      ? "completed"
      : isOverdue
...
async function buildAssignmentDetails(assignmentId: number) {
  ...
  const [targetUser, trainer, assigner, items, progressRows] =
    await Promise.all([
      getUserById(assignment.targetUserId),
      assignment.trainerUserId
        ? getUserById(assignment.trainerUserId)
  ...
  return {
    ...
    targetUser: targetUser
      ? {
          ...
          subBranchAdminId: targetUser.subBranchAdminId,
        }
      : null,
...
export const onboardingAssignmentsRouter = router({
  assign: teamLeaderOrAboveProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!template || !template.isActive || template.archivedAt)
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "활성 템플릿만 배정할 수 있습니다.",
        });
      if (
        ctx.user.role !== "branch_admin" &&
        template.targetRole !== targetUser.role
      ) {
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "대상 역할과 템플릿 역할이 일치하지 않습니다.",
        });
      }
      if (input.trainerUserId)
        await assertCanManageTarget(ctx.user, input.trainerUserId);
      ...
      await createOnboardingItemProgressRows(
        items.map(item => ({
  ...
  list: managerAnalyticsProcedure
    ...
    .query(async ({ ctx, input }) => {
      const scopeUserIds =
        ctx.user.role === "branch_admin"
          ? undefined
          : ((await getHierarchyScopeUserIds(ctx.user)) ?? [ctx.user.id]);
      ...
      return Promise.all(
        assignments.map(assignment => buildAssignmentDetails(assignment.id))
      );
    }),
  ...
  updateItemProgress: activeUserProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (ctx.user.id !== assignment.targetUserId)
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "본인 항목만 완료 체크할 수 있습니다.",
        });

      const itemRows = await getOnboardingTemplateItems(
      ...
      const progressRows = await getOnboardingItemProgressByAssignment(
        input.assignmentId
      );
      const progress = progressRows.find(row => row.itemId === input.itemId);
      if (!progress) throw new TRPCError({ code: "NOT_FOUND" });

      const nextStatus = item.requiresManagerApproval
        ? "needs_approval"
        : "approved";
  ...
  approveItem: teamLeaderOrAboveProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!progress) throw new TRPCError({ code: "NOT_FOUND" });
      const assignment = await getOnboardingAssignmentById(
        progress.assignmentId
      );
      if (!assignment) throw new TRPCError({ code: "NOT_FOUND" });
      await assertCanReadAssignment(ctx.user, assignment);
      if (progress.status !== "needs_approval")
        throw new TRPCError({
          code: "BAD_REQUEST",
          message: "승인 대기 항목만 처리할 수 있습니다.",
        });
  ...
  summary: managerAnalyticsProcedure.query(async ({ ctx }) => {
    ...
    const statusCounts = {
      ...
      approvalPending: assignments.reduce(
        (sum, item) => sum + (item.approvalPendingCount ?? 0),
        0
      ),
      requiredIncomplete: assignments.reduce(
        (sum, item) => sum + (item.requiredPendingCount ?? 0),
        0
      ),
    };
    const byRole = {
    ...
    const users = await Promise.all(
      assignments.map(item => getUserById(item.targetUserId))
    );
